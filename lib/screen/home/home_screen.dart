...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget _buildHomeContent(AuthProvider auth, JobProvider jobProv) {
    final visibleJobs = auth.role == 'job_poster' ? jobProv.jobs : jobProv.publishedJobs;
    return SafeArea(
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            _buildSuggestedJobs(visibleJobs, auth, jobProv),
            _buildSectionHeader('Việc làm mới nhất'),
            _buildLatestJobs(auth, jobProv),
  ...
  Widget _buildHeader(AuthProvider auth) {
    final nameFromEmail = auth.user?.email?.split('@').first ?? 'Khách';
    final displayName = (auth.fullName != null && auth.fullName!.isNotEmpty)
        ? auth.fullName!
        : nameFromEmail;
    return Padding(
  ...
  Widget _buildSuggestedJobs(List<JobModel> jobs, AuthProvider auth, JobProvider prov) {
          ...
          child: Scrollbar(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return GestureDetector(
                  onTap: () => _showFilteredJobList(context, auth, prov, job.title),
  ...
  void _showFilteredJobList(
    ...
    final filteredJobs = prov.filterJobs(keyword, publishedOnly: auth.role != 'job_poster');
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) =>
  ...
  Widget _buildLatestJobs(AuthProvider auth, JobProvider prov) {
    if (prov.isLoading) return const Center(child: CircularProgressIndicator());
    final jobs = auth.role == 'job_poster' ? prov.jobs : prov.publishedJobs;
