...
class JobProvider extends ChangeNotifier {
  ...
  List<JobModel> get publishedJobs {
    final base = _jobs.where((job) => job.status == 'published');
    if (_searchTerm.isEmpty) return base.toList();
    final term = _searchTerm.toLowerCase();
  ...
  List<JobModel> filterJobs(String term, {bool publishedOnly = false}) {
    final source = publishedOnly ? _jobs.where((job) => job.status == 'published') : _jobs;
    if (term.isEmpty) return source.toList();
